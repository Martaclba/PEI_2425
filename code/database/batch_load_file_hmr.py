...
BATCH_SIZE = 20000  # Define batch size
        ...
        for df in tqdm.tqdm(df_chunks):
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if product is None:
                        # Generating a CNP within the valid range for integer
                        cnp_value = random.randint(1, 2147483647)
                        cur.execute("INSERT INTO product (CNP, name, fk_id_Company) VALUES (%s, %s, %s) RETURNING CNP",
                                    (cnp_value, row['Product'].strip(), company_id))
                        product = cur.fetchone()
                    product_id = product[0]

                    # Check and insert into `HMR_Zone` (make sure `brick` exists in `hmr_zone`)
                    ...
                    # Insert into `Sale` and fetch sale_id (ensure fk_brick exists)
                    cur.execute("""
