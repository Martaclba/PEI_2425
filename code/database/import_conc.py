...
# Load processed CSV
# Paths
detector_script = "../data_treat/detector.py"  # Validation script
input_file = "--/data_treat/processa_hmr_concorrencia_mensal.py"  # Generates the cleaned CSV
cleaned_csv = "output.csv"  # File generated by detetor.py

# Step 1: Run the input script to generate the cleaned data
print(f"Running input script: {input_file} for file {args.path}...")
try:
    subprocess.call(["python", input_file, args.path], check=True)
...
try:
    result = subprocess.call(["python", detector_script], capture_output=True, text=True, check=True)
                ...
                if product_name:
                    ...
                    if product_row is None:
                        cur.execute(f"""
                            ...
                            VALUES (DEFAULT, '{product_name}', {company_id}) 
                            RETURNING cnp;
                        """)
                        product_row = cur.fetchone()
